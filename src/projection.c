...
t_ray	*make_ray(double x, double y, t_data	*info)
{
	t_vec3d	*forward;
	t_vec3d	*right;
	t_vec3d	*up;
...
void	projection(t_data *info)
{
	...
	// h = 2 * tan(info->cam->FOV * M_PI / 180 * 0.5);
	// w = h * ASPECT_RATIO;
	// printf("H %f W%f\n", h, w);
	tmp = info->obj;
	while (x_pixel <= SCREEN_WIDTH)
	{
		...
		{
			x = ((2 * (x_pixel + 0.5) / (double)SCREEN_WIDTH - 1) * tan((info->cam->FOV * M_PI / 180))) * ASPECT_RATIO;
			y = (1 - 2 * ((y_pixel + 0.5) / (double)SCREEN_HIGHT)) * tan((info->cam->FOV * M_PI / 180));
			ray = make_ray(x, y, info);
			y_pixel++;
			tmp = info->obj;
			while (tmp)
			{
				if (tmp->id == 1 && sphere_intersection(ray, tmp->sphere) < 0)
					my_mlx_pixel_put(info, x_pixel, y_pixel, 0x00ff00ff);
				tmp = tmp->next;
			}
		}
		x_pixel++;
